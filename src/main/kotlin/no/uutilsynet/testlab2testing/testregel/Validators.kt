...
fun validateName(name: String?): Result<String> = runCatching {
  if (name.isNullOrBlank()) {
    throw IllegalArgumentException("Namn kan ikkje vera blank")
  }
  name
}
...
fun validateSchema(testregelSchema: String?, type: TestregelType): Result<String> = runCatching {
  ...
  } else if (type == TestregelType.inngaaende && validateJSONString(testregelSchema).isFailure) {
