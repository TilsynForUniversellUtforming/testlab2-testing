...
class MaalingResultResource(
  ...
  fun getTestresultatForMaaling(@PathVariable maalingId: Int): List<Testresult> {
    ...
    val testresultat =
        resultatService.getKontrollResultatByKontrollId(kontrollId).flatMap { it.loeysingar }
    ...
    return if (maaling is Maaling.TestingFerdig) {
      ...
      testkoeyringar.map {
        ...
        val overallCompliancePercent = calculateOverallCompliancePercentage(results)
        Testresult(
  ...
  fun getTestresultatForLoeysing(
    ...
    return aggregeringService.getAggregertResultatTestregel(maalingId).filter {
      it.loeysing.id == loeysingId
    }
  }
