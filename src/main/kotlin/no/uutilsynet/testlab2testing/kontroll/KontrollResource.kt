...
class KontrollResource(
  ...
  fun getKontroller(): List<KontrollListItem> {
    return kontrollService.getKontroller().getOrElse {
  ...
  fun getKontroll(@PathVariable id: Int): ResponseEntity<Kontroll> {
    return kontrollService
        .getKontrollAsResult(id)
        .fold(
  ...
  fun deleteKontrollData(
    ...
    return runCatching { kontrollCleanupService.cleanupKontrollData(kontrollId) }
        .fold(
            onSuccess = { ResponseEntity.noContent().build() },
            onFailure = {
