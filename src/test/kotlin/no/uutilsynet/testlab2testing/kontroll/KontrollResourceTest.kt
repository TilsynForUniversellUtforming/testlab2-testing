...
class KontrollResourceTest(
    ...
    fun getKontrollById() {
        val body = kontrollInitBody
        val location =
            createDefaultKontroll(body)
        val responseBody = client.get().uri(location).exchange().expectStatus().isOk
    ...
    fun updateKontrollWithLoeysingar() {
        ...
        val utvalLocation = client.post().uri("/v1/utval").body(nyttUtval).exchange().expectStatus().isCreated
            .returnResult().responseHeaders.location!!

        val utval = client.get().uri(utvalLocation).exchange().expectStatus().isOk
            .returnResult<Utval>().responseBody!!
    ...
    fun updateKontrollWithTestreglarRegelsett() {
        ...
        val regelsettLocationForId = client
            .post()
            .uri("/v1/regelsett")
            .body(nyttRegelsett)
            .exchange()
        ...
        val regelsett = client.get().uri("http://localhost:$port$regelsettLocationForId")
            .exchange().expectStatus().isOk
            .returnResult<Regelsett>().responseBody!!
    ...
    private fun getOpprettetKontroll(location: URI): Kontroll {
        val opprettetKontroll = client.get().uri(location).exchange().expectStatus().isOk
            .returnResult<Kontroll>().responseBody!!
    ...
    private fun createDefaultKontroll(body: Map<String, String>): URI {
        val location = client.post().uri("/kontroller").body(body).exchange().expectStatus().isCreated
            .returnResult().responseHeaders.location!!
        return location
    }
    ...
    fun getKontrollerTest() {
        doReturn(Result.success(listOf(testregel))).`when`(testregelClient).getTestregelList()
        doReturn(Result.success(listOf(loeysingList[0])))
            .`when`(loeysingsRegisterClient).getMany(listOf(loeysingList[0].id))
        RestAssured.defaultParser = Parser.JSON


        val kontroller = client.get().uri("/kontroller").exchange().expectStatus().isOk
            .returnResult<Array<KontrollResource.KontrollListItem>>().responseBody!!
    ...
    fun getKontrollStatus() {
        ...
        val status = client.get().uri(statusUrl).exchange().expectStatus().isOk
            .returnResult<TestStatus>().responseBody!!
