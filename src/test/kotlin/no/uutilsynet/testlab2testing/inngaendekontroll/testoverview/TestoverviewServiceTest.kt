...
class TestoverviewServiceTest(@Autowired val testoverviewService: TestoverviewService) {
    ...
    @MockitoBean
    private lateinit var testgrunnlagService: TestgrunnlagService
    @MockitoBean
    private lateinit var loeysingsRegisterClient: LoeysingsRegisterClient
    @MockitoBean
    private lateinit var statisticsService: TestOverviewStatisticsService
    @MockitoBean
    private lateinit var styringsdataService: StyringsdataService
    @MockitoBean
    private lateinit var testregelClient: TestregelClient
    @MockitoBean
    private lateinit var testResultatDAO: TestResultatDAO
    @MockitoBean
    private lateinit var kontrollDAO: KontrollDAO

    private val loeysingId = 1
    private val kontrollId = 10
    private val testgrunnlagId = 100

    private fun loeysing() = Loeysing(
        id = loeysingId,
        namn = "Test Loeysing",
    ...
    private fun sideutval(id: Int = 1) = Sideutval(
        id = id,
    ...
    private fun kontrollDB() = KontrollDAO.KontrollDB(
        id = kontrollId,
    ...
    private fun testStatusCount() = TestStatusCount(
        loeysingId = loeysingId,
    ...
    private fun resultat(status: Status = Status.IkkjePaabegynt) = ResultatManuellKontroll(
        id = 1,
        testgrunnlagId = testgrunnlagId,
        loeysingId = loeysingId,
        testregelId = 1,
    ...
    fun `listTestOverviewElements returns TestingStatus for each loeysing`() {
        ...
        `when`(kontrollDAO.getKontroller(listOf(kontrollId))).thenReturn(Result.success(listOf(kontrollDB())))
        `when`(testgrunnlagService.getTestgrunnlagForKontroll(kontrollId)).thenReturn(testgrunnlagList)
        `when`(loeysingsRegisterClient.getMany(listOf(loeysingId))).thenReturn(Result.success(listOf(loeysing())))
        ...
        `when`(styringsdataService.getStyringsdataForLoeysing(loeysingId, testgrunnlagId)).thenReturn(
            styringsdataElement()
        )
    ...
    fun `status is FERDIG when all results are Ferdig`() {
        ...
        `when`(testgrunnlagService.getTestgrunnlagForKontroll(kontrollId)).thenReturn(testgrunnlagList)
        `when`(loeysingsRegisterClient.getMany(listOf(loeysingId))).thenReturn(Result.success(listOf(loeysing())))
        `when`(testResultatDAO.getManyResults(testgrunnlagId)).thenReturn(Result.success(ferdigResultat))
        `when`(
            statisticsService.getTestingStatusForLoeysing(
                loeysingId,
                testgrunnlagId,
    ...
    fun `kanSlette is false for RETEST with existing resultat`() {
        ...
        `when`(testgrunnlagService.getTestgrunnlagForKontroll(kontrollId)).thenReturn(testgrunnlagList)
        `when`(loeysingsRegisterClient.getMany(listOf(loeysingId))).thenReturn(Result.success(listOf(loeysing())))
        `when`(testResultatDAO.getManyResults(testgrunnlagId)).thenReturn(Result.success(existingResultat))
        `when`(
            statisticsService.getTestingStatusForLoeysing(
                loeysingId,
                testgrunnlagId,
    ...
    fun `listTestOverviewElements aggregates results across opprinneligTest and restestar`() {
        ...
        `when`(kontrollDAO.getKontroller(listOf(kontrollId))).thenReturn(Result.success(listOf(kontrollDB())))
        `when`(testgrunnlagService.getTestgrunnlagForKontroll(kontrollId)).thenReturn(testgrunnlagList)
        `when`(loeysingsRegisterClient.getMany(listOf(loeysingId, loeysingId))).thenReturn(
            Result.success(
                listOf(
                    loeysing()
                )
        ...
        `when`(styringsdataService.getStyringsdataForLoeysing(loeysingId, retestId)).thenReturn(styringsdataElement())
